...
void RecogniseCommandState::sendData(WiFiClient *wifiClient, HTTPClient *httpClient, const char *url, uint8_t *bytes, size_t count)
{
...
bool RecogniseCommandState::run()
{
    ...
    if (sample_count > 0)
    {
        ...
        while (sample_count > 0)
        {
            ...
            Serial.println(123);
            Serial.println(std::min(sample_count, 500) * 2);

            sendData(wifiClientI2S, httpClientI2S, I2S_SERVER_URL, (uint8_t *)sample_buffer, std::min(sample_count, 500) * 2);
            Serial.println(456);
